...
class CaracteristicaSURF:
    ...
    def __init__(self, train_imagen = "carta_fin.png"):
    ...
    def matching(self, imagen):
        ...
        #Rechazar puntos fuera de la imagen
        #if np.any(filter(lambda x:x[0] < -20 or x[1] < -20 or x[0] > tamano_in[1]+20 or x[1] > tamano_in[0]+20,dst_bordes)):
        #    self.no_exitoso_frames = self.no_exitoso_frames + 1
            #return False, imagen
        ...
        #Rechazar puntos de borde si el area es ilogica
        #if area < np.prod(tamano_in)/16. or area > np.prod(tamano_in)/2.:
            #self.no_exitoso_frames = self.no_exitoso_frames + 1
            #return False, imagen
        ...
        imag_flann = dibujar_buenos_p(self.imagen_obj, self.key_train, imagen_in, key_in, buenos_puntos)
        ...
        #Rechazar puntos
        reciente = self.no_exitoso_frames < self.no_exitoso_f_max
        similar  = np.linealg.norm(Hinv-self.last_hinv) < self.error_max_h
        #if reciente and not similar:
            #self.no_exitoso_frames = self.no_exitoso_frames + 1
            #return False, imagen
    ...
    def dibujar_buenos_p(img1,kp1,img2,kp2,matches):
